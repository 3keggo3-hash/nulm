...
class SnapshotManager:
    ...
    def _collect_plan_state(self) -> list[dict[str, Any]]:
        plans = self._plans_dir()
        if plans is None:
            return []
    ...
    def create_git_checkpoint(self, name: str) -> dict[str, Any]:
        ...
        add_result = self._run_git(["git", "add", "-A"])
        ...
        commit_result = self._run_git(["git", "commit", "-m", f"checkpoint: {name}"])
        ...
        plan_state = self._collect_plan_state()
        timestamp = datetime.now(timezone.utc).isoformat()
    ...
    def restore_git_checkpoint(self, name: str) -> dict[str, Any]:
        ...
        verify_result = self._run_git(["git", "cat-file", "-t", commit_hash])
        ...
        checkout_result = self._run_git(["git", "checkout", str(commit_hash)])
