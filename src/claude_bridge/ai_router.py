...
def _validate_base_url(provider: str, base_url: str) -> None:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise ValueError(f"{provider} base_url must be an HTTP(S) URL")
    if provider == "ollama":
        if not _is_loopback_host(parsed.hostname):
            raise ValueError("ollama base_url must point to localhost or a loopback address")
        return
    if parsed.scheme != "https":
        raise ValueError(f"{provider} base_url must use https")
    from claude_bridge.url_tools import _is_private_host, _resolve_and_check_host

    if _is_private_host(parsed.hostname):
        raise ValueError(f"{provider} base_url must not point to a private/internal host")
    if _resolve_and_check_host(parsed.hostname) is not None:
        raise ValueError(f"{provider} base_url resolves to a private/internal host")
